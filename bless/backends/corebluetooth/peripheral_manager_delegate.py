...
if os.environ.get("BLESS_DOCS_BUILD"):
    ...
    class PeripheralManagerDelegate(  # type: ignore # noqa: F811
        ...
        async def start_advertising(
                ...
                if num_uuids > 0:
                    if len_local_name >= 10:
                        LOGGER.warning(
                            ...
                            + "may not be transmitted appropriately because it is "
                            + "longer than the available space. Either remove the "
                            + "UUIDs being advertised or shorten the local name to "
                    ...
                    if len_local_name >= 28:
                        LOGGER.warning(
                            "The local name of your BLE application "
                            + "may not be transmitted appropriately because it is "
                            + "longer than the available space. Consider advertised "
                            + "shortening the local name to less than 28 characters."
                        )
            ...
            LOGGER.debug(
                "Advertising started with the following data: {}".format(
                    advertisement_data
                )
            )
        ...
        def peripheralManager_didAddService_error(  # noqa: N802
            ...
            LOGGER.debug(
                "service added had characteristics: {}".format(
